...
class Collect_pic(QWidget):
    ...
    def take_pic_home(self, pichome='pichome',picname=''):
        if picname=='':
        ...
        self.trayicon.showMessage('get pics','%d %s %s %s'%(piclen,pichex,picname,existed),QSystemTrayIcon.Information,3000)
        print piclen,pichex,picname
    ...
    def on_rename_files(self):
        t_rename_files = QFileDialog.getOpenFileNames(self,
                                                      ...
                                                      '',
                                                      'Images (*.jpg *.bmp *.png)')
        if not os.path.exists('robpics'):
            os.mkdir('robpics')
        for _ef in t_rename_files:
            self.take_pic_home('robpics',_ef)
